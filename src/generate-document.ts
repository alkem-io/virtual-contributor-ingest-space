...
export default (docLike: any): GeneratedDocument => {
  const { id, type, profile: { references, tagset, url: source, description, tagline, displayName, location, visuals }, context } = docLike;
  ...
  if (tagset?.tags.length) document = `${document}\nTags: ${tagset?.tags.join(', ')}`
  ...
  if (postalCode || city || country) document = `${document}\nLocation: ${postalCode} ${city} ${country}`
